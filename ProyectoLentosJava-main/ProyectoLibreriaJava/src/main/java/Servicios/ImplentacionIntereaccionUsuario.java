...
public class ImplentacionIntereaccionUsuario implements InterfaceIntereccionUsuario {
	...
	public boolean OlvidarClaveUsuario(String correo) {
		...
		try {
			...
			UsuarioDTO usuario = HacerGets("usuarioDNI/"+correo);
			if(usuario!=null) {
				UUID uuid = UUID.randomUUID();
				String token = uuid.toString();
				Calendar fechalimiteToken = Calendar.getInstance();		
				fechalimiteToken.add(Calendar.MINUTE, 10);
				TokenDTO tokendto=new TokenDTO(token,fechalimiteToken,usuario.getIdUsuario());
